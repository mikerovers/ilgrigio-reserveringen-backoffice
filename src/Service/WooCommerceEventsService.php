        ...
        try {
          ...
          // Transform the events to the expected format
            ...
            foreach ($eventsWithProducts as $eventData) {
              // Skip events without proper product data or if product is out of stock
                if (
                    !isset($eventData['product']) ||
                    empty($eventData['product']['stock_status']) ||
                    $eventData['product']['stock_status'] !== 'instock'
                ) {
                    $this->logger->debug('Skipping event with no product or out of stock', [
