    ...
    public function downloadPdf(string $token): Response
    {
        try {
            ...
            if (!$this->securePdfStorageService->isValidToken($token)) {
                $this->logger->warning("Invalid PDF view token accessed", [
                    "token" => substr($token, 0, 8) . "...", // Log only first 8 chars for security
                ]);
                throw $this->createNotFoundException(
                    "PDF not found or token invalid",
                );
            }

            // Get order data for filename
            $orderData = $this->securePdfStorageService->getOrderDataByToken(
                $token,
            );
            if (!$orderData) {
            ...
            $this->logger->error("PDF view failed", [
                "token" => substr($token, 0, 8) . "...",
                "error" => $e->getMessage(),
            ]);
