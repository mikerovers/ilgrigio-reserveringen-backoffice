...
class TestController extends AbstractController
{
    ...
    public function testOrderPdf(): Response
    {
        ...
        $sampleOrderData = [
            ...
            "line_items" => [
                [
                    "name" =>
                        "Bombolini - dinsdag 15 juli - ochtend (3 t/m 12 jaar)",
                    "quantity" => 1,
                    "price" => "15.00",
                    "total" => "15.00",
                ],
    ...
    public function testTicketPdf(): Response
    {
        ...
        try {
            ...
            return new Response($pdfContent, 200, [
                ...
                "Content-Disposition" =>
                    'inline; filename="ticket-bombolini-mock.pdf"',
            ]);
        } catch (\Exception $e) {
            return new JsonResponse(
                ["error" => $e->getMessage()],
        ...
        foreach ($mockTicketInfo["tickets"] as $ticketId => &$ticket) {
            if (isset($ticket["ticket_code"])) {
                ...
                if (isset($ticket["ticket_name"])) {
                    $ticket[
                        "short_ticket_name"
                    ] = $this->ticketNameService->generateShortTicketName(
                        $ticket["ticket_name"],
                    );
                }
                // Generate QR code
    ...
    private function generateMockQrCode(array $ticketData): string
    {
        try {
            ...
            if (isset($ticketData["ticket_name"])) {
                $shortTicketName = $this->ticketNameService->generateShortTicketName(
    ...
    public function testPdfDirect(): Response
    {
        ...
        $sampleOrderData = [
            ...
            "meta_data" => [
                ["key" => "_event_name", "value" => "Bombolini - dinsdag 15 juli - ochtend"],
                ["key" => "_event_date", "value" => "2025-07-15"],
                ["key" => "_event_time", "value" => "14:00"],
            ],
        ];

        try {
            $pdfContent = $this->orderPdfService->generateOrderPdf(
                $sampleOrderData,
            );
