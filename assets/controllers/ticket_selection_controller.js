...
export default class extends Controller {
    ...
    validateForm(event) {
        ...
        // Validate individual ticket quantities
        let hasError = false
        this.quantityTargets.forEach(input => {
            if (!input.disabled) {
    ...
    updateOrderSummary() {
        ...
        this.subtotalPriceTarget.textContent = `€${subtotalPrice.toFixed(2)}`
        ...
        if (this.hasMaxTicketsWarningTarget) {
            if (totalTickets >= this.maxTicketsPerOrderValue - 5) { // Show warning when within 5 tickets of limit
                ...
                if (totalTickets >= this.maxTicketsPerOrderValue) {
                    ...
                    this.maxTicketsWarningTarget.classList.remove('text-gray-500', 'bg-gray-50')
                } else {
