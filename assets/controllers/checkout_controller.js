...
export default class extends Controller {
    ...
    updateTotals() {
        ...
        let discount = this.discountAmountValue
        
        // If we have an applied coupon but no discount amount from backend, calculate it
        if (this.appliedCoupon && discount === 0) {
            if (this.appliedCoupon.discount_type === 'percentage') {
                discount = (subtotal * this.appliedCoupon.amount) / 100
            } else {
        ...
        const tax = totalAfterDiscount - (totalAfterDiscount / (1 + taxRate))
        const subtotalWithoutTax = totalAfterDiscount - tax
